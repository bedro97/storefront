...
/**
 * Bulk Add Products Script for Saleor (Final Version)
 *
 ...
 *
 * Usage:
 *   node scripts/bulk-add-products-final.mjs
 *
...
const generateUniqueSKU = (baseSKU) => {
...
async function createProduct(productData, channelId, categoryId, productTypeId) {
	...
	// First, assign the product to the channel
	const assignProductMutation = `
	...
	const assignResult = await makeGraphQLRequest(assignProductMutation, {
		id: product.id,
		input: {
			updateChannels: [
				{
	...
	if (assignResult.productChannelListingUpdate.errors.length > 0) {
		throw new Error(
			`Failed to assign product to channel: ${JSON.stringify(
				assignResult.productChannelListingUpdate.errors,
			)}`,
		);
	}

	console.log(`✅ Product assigned to channel`);
		...
		input: [
			{
				...
				costPrice: Math.round(productData.price * 0.6 * 100) / 100, // 40% margin, rounded to 2 decimal places
			},
		],
	});
